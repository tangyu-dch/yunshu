...
### 🙇‍♂️ 开发者致歉
在此前版本中，本项目的描述文档（README）由于编写疏忽，错误地掺杂了面向开发者内部的高强度开发约束、代码规范和纪律条约，导致文档结构混乱，给外部审阅者和开源用户带来了不必要的阅读负担与困惑。为此，我们向所有关注和使用本项目的伙伴们表达最诚挚的歉意！

目前，该文档已完成全面优化与提纯，所有开发纪律已妥善归档于专用于开发指导的 [AGENTS.md](file:///Users/tangyu/Projects/yunshu/AGENTS.md) 文件中。未来的 README 将始终秉持纯净的原则，仅保留项目架构、核心特色与开发部署指引。
...
3. **无担保承诺**：本开源软件提供“按原样”的无担保服务，不作任何明示或暗示的保证（包括但不限于对适销性、特定用途的适用性或非侵权性的担保）。使用方需自行承担本系统运行可能带来的通信信道开销及硬件资源负载风险。
