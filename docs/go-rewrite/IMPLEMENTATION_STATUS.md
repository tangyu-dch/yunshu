...
- cc-console now implements Kamailio dispatcher management for `/operate/kamailio/dispatcher` with page, detail, add, update, delete, and reload endpoints backed by a Go-native `cc_res_freeswitch` table.
...
- cc-call now maps Kamailio subscriber authentication directly to the unified `cc_res_extension` table, supporting secure multi-tenant auth with pre-calculated `ha1`/`ha1b` digests and Redis auth-cache invalidation.
