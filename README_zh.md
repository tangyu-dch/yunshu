...
### 4. 部署模式：私有化部署（单商户模式）与 SaaS 多商户模式

云枢开箱即用支持**私有化部署（单商户模式）**与 **SaaS 多租户模式**。您可以通过配置文件（`configs/default.yaml` 或 `configs/production.yaml`）中的 `tenant` 节点来控制系统的运行行为：

```yaml
tenant:
  mode: "single"             # 可选值："single"（私有化单商户部署）或 "multi"（SaaS 多商户模式）
  defaultMerchantId: 1001    # 私有化单商户模式下的默认商户 ID（用于自动降级与上下文自动补全）
```

#### 📊 核心区别一览

| 特性 / 维度 | 🏢 单商户模式（私有化部署） | ☁️ SaaS 多商户模式（云平台运营） |
| :--- | :--- | :--- |
| **主要应用场景** | 企业内部独立呼叫中心、局域网本地化部署、单主体话务运营。 | 商业化 SaaS 平台运营、云客服托管服务商。 |
| **商户身份识别** | 登录会话与 API 上下文自动补充为 `defaultMerchantId`（`1001`）。 | 显式的商户登录选择、独立的租户权限校验与账户账单。 |
| **控制台功能收拢** | 自动隐藏或锁定“商户管理”（新增/删除）及“多租户域名绑定”等运营入口。 | 解锁完整的商户准入审批、多渠道充值流水、租户隔离路由看板。 |
| **数据隔离机制** | 单一数据库主体，默认不进行租户级物理及行级隔离校验。 | 基于 `merchant_id` 的数据库行级逻辑隔离、独立 Redis 缓存与 AI 流程画布隔离。 |
| **分机注册与鉴权** | 单 SIP 域名注册。同名分机无冲突风险，无需进行域绑定。 | 支持多域隔离（Kamailio `use_domain=1`），分机通过 `ha1b` 带域绑定鉴权，允许不同商户注册同名分机（如 `1001`）。 |
| **计费与余额校验** | 计费主要用于流量审计与估算，可选择不强制余额扣减和欠费锁定。 | 严密的实时计费与扣减，高并发 Redis Lua 脚本原子防透支锁定，欠费自动拦截呼叫。 |
| **资源路由划拨** | 主/被叫号码池、ACD 技能队列、中继网关直接应用给默认商户，无需分配。 | 运营商中继与号码资源必须由运营后台统一划拨给特定商户，并受商户并发限制约束。 |

---

#### 🏢 私有化部署（单商户模式）
专为企业内部独立客户服务中心、私有化本地部署或单商户运营环境设计：
*   **身份自动补充与拦截降级**：所有控制台登录会话和 API 外呼请求会自动填充为指定的 `defaultMerchantId`（默认 `1001`），完全绕过 SaaS 下的商户选择界面和分段隔离校验，保证单商户业务链路零感知流畅运行。
*   **收拢并屏蔽 SaaS 物理边界**：前端控制台会自动感知隔离模式。在单商户模式下，**全局自动隐藏或锁定“商户管理”（新增/删除商户）、“多租户域名绑定”等运营级管理入口**，优化工作台布局，防范误操作导致唯一的商户主体被破坏。
*   **极简话务与号码路由分配**：所有的主叫/被叫号码池、ACD 呼叫技能组、SIP 注册域名等电信级资源，默认且自动全局应用给默认商户 1001，无需管理员手动配置复杂的多商户分配关系，开箱即用。
*   **默认种子自动播种**：如果在系统首次运行初始化时，数据库中检测不到该默认商户配置，系统种子管理器会自动播种生成默认的商户主体资质，并配置好默认的分机 AppKey/Secret 键值对，极大降低首次部署调试门槛。

#### ☁️ SaaS 多商户运营模式
当将 `tenant.mode` 设置为 `"multi"` 时，云枢将全面激活多租户物理隔离防护网：
*   **多商户数据物理隔离**：各商户间的数据、分机（支持同名分机通过 `ha1b` 进行域绑定隔离鉴权）、SIP 网关限额、大模型 API 凭证、AI 语音画布流以及预付费账户余额相互物理隔离，互不干扰。
*   **全功能运营管理台**：解锁完整的商户资质准入控制、发票充值结算明细、高并发号码源按比例分发绑定、跨商户拦截审计以及全局防火墙风控策略。
...
### 第二阶段：高并发选号策略与分布式事件租约 [100% 已完工]
*   ✅ **高并发逐个试选选号系统**：基于 Redis 规则链的并发控制，在批量呼起时对候选号码进行逐个试选直至分配成功。
*   ✅ **振铃音与早期媒体编排**：媒体编排器（MediaOrchestrator）管理运营商 183 早期媒体与补振铃音播放，支持 broadcastTime 定时器自动停播。
*   ✅ **选号租约看门狗**：后台周期扫描活跃选号 claim Key 并续期 TTL，防止长通话超时导致并发计数器过期。
*   ✅ **FreeSWITCH ESL 节点租约机制**：多实例 `cc-call` 部署时通过 MySQL CAS 行锁（`cc_res_fs_lease` 表）竞争领取 FS 节点事件监听租约，半 TTL 周期续约，断连自动释放，防范重复消费。

### 第三阶段：账务高安全与独立结算工作流解耦 [100% 已完工]
*   ✅ **CDR 信道可靠 Outbox**：基于 MySQL 本地出件箱持久化暂存 CDR 话单挂断基础事实。
*   ✅ **独立计费与费率审计**：商户专属费率解析器（GORM + Redis 缓存），支持动态计费颗粒度与完整审计链。
*   ✅ **原子余额防超扣**：Redis Lua 脚本原子扣款，支持信用额度透支控制与结算台账集成。

### 第四阶段：高可靠异步 Worker 结转与下游推送 [100% 已完工]
...
*   ✅ **异步录音文件 CDN 转储**：S3 兼容录音上传（MinIO/阿里云 OSS），CDN 公网访问映射，失败自动 outbox 重试。

### 第五阶段：动态权限完整落库与控制台安全 [100% 已完工]
